...
本目录将公开 Notion 系列整理为十一周中文主笔记。内容保留每周的主题、公式、系统数据流、作业方向和稳定外部参考，但不复制 Notion 界面、附件或临时下载链接。英文目录保存对应的纯英文结构化版本。
...
课程周次与现有专题讲义不是一一替代关系。周笔记保留课程组织，`docs/chapters/` 则按本知识库的数学定义、reference、CUDA 接口与验证边界重新组织专题。
